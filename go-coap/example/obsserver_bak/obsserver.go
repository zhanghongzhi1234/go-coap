	...
	log.Fatal(coap.ListenAndServe("udp", ":5683",
		coap.FuncHandler(func(l *net.UDPConn, a *net.UDPAddr, m *coap.Message) *coap.Message {
			...
			if m.Code == coap.GET {
				...
				//original code is wrong, actually temp1 is uint32 type, parseOptionValue() call decodeInt() in message.go return unit32
				/*temp1 := m.Option(coap.Observe)
				if value, ok := temp1.([]uint8); ok &&
				...
				if err != nil {
					log.Fatalln("Could not convert observe value to int", err)
				}
				if value == 1 {
					go periodicTransmitter(l, a, m)
				}*/
